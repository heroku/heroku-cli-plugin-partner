...
export default class Enroll extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    this.log(`Enrolling add-on ${color.addon(addonId)} to partner integration ${color.name(idOrSlug)}...`)
    ...
    try {
      ...
      switch (connErr.body?.id) {
        ...
        default: {
          this.error(
            `We can't enroll add-on ${color.addon(addonId)} to partner integration ${color.name(idOrSlug)} due to an unexpected error.`
          )
        }
      }
    }
    ...
    this.log(
      `✓ Successfully enrolled add-on ${color.addon(enrollResponse.addon_uuid)} to partner integration ${color.name(idOrSlug)}.`
    )
